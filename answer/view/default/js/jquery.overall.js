...
//----------ytl 返回列表时输入框的默认值处理 sor：选择器，text：onblur的默认值---------------------------
function b_inputTipText(sor,text){     
	$(sor)
	   .css({"color":"#888"})     //灰色
	   .focus(function(){  
		if($(sor).val()!=text){$(this).css({"color":"#000"})}else{$(sor).val("").css({"color":"#888"})}
	   })
	   .blur(function(){
		if($(sor).val()==""){$(this).val(text).css({"color":"#888"})}
	   })
	   .keydown(function(){$(sor).css({"color":"#000"})})
}
...
$(document).ready(function(){
	// 选项卡最后一个li加右边框
	$('.tab').find('.tab-label').find('li').append('<span class="tab_left"></span><span class="tab_right"></span>');
});