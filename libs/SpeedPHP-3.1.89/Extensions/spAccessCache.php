...
class spAccessCache{
	...
	public function __call($name, $args){
		...
		if(!class_exists($driverClass))spError($args.'spAccess无法找到名为{$name}缓存驱动程序，请检查!');
		
		//modify by weijuan 2011-12-30
		$ary = array('method'=>$args[0],'value'=>$args[1]);	
		...
		if('w' == $method){ // 写数据
			...
			return spClass($driverClass)->get($value);
		}
	}
}
